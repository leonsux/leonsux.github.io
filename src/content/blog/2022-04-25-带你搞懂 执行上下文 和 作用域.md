...
description: "执行上下文（Execution Context） 每当 Javascript 代码在运行的时候，它都是在执行上下文中运行，用来跟踪记录代码运行时环境的抽象概念。 执行上下文的三种类型： 全局执行上下文 任何不在函数内部的代码都在全局环境中 一个程序只有一个全局执行上下文 全局环境"
...
draft: false
featured: false
permalink: "/2022/04/25/带你搞懂-执行上下文-和-作用域/"
legacy: true
---
